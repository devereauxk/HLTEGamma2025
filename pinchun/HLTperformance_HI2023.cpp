...
//void HLTperformance_HI2023(int trigType)
  ...
  */
  ...
  if (fileList != "") {
    ...
    while (getline(infile, line) && (fileCount < nfiles || nfiles == -1)) {
      if(fileCount%100==0)
        cout<<"Adding file "<<fileCount<<": "<<line.c_str()<<endl;
    ...
    for(int iF = 0; iF < frtfile.size(); iF++){
      ...
      auto filelist = my_glob(frtfile[iF].c_str());
      cout<<"number of files found: "<<filelist.size()<<endl;
  ...
  for (Long64_t j_entry = 0; j_entry < entriesTmp; ++j_entry){
    ...
    if(isEle&&isMC){
      for(long unsigned int iele=0;iele<size(*ggHi.elePt);iele++){
        ...
        for (unsigned igen = 0; igen < size(*ggHi.mcEta); ++igen) {
    ...
    for(int i=0; i < num_of_PE; ++i) {
      if(isMC){
        ...
        if(isEle){
          ...
          if (std::abs(pid) != 11) continue; // || std::abs(mpid) != 23) continue;
        }else{
          genID = (*ggHi.pho_genMatchedIndex)[i];
          if (genID == -1) { continue; }
          auto pid = (*ggHi.mcPID)[genID];
          //auto mpid = (*ggHi.mcMomPID)[genID];
          if (pid != 22) continue; // || (std::abs(mpid) > 22 && mpid != -999)) continue;
    ...
    for(int iC=0;iC<4;iC++){
      ...
      //Is Barrel
      if(!isEle && (abs((*ggHi.phoSCEta)[maxPt_i]) >= 1.442||(*ggHi.phoEt)[maxPt_i]<50))
        continue;
    ...
    for(long unsigned int iT = 0; iT < triggers.size(); iT++){
      ...
      if(sel_cut&&sel_cut2){
        float n_pho_ho = 0;
        float drtemp;
        drTrig=0; // KD skip HLTobj matching
